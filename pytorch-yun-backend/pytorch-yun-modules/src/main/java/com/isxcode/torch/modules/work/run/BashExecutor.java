...
public class BashExecutor extends WorkExecutor {
    ...
    public void execute(WorkRunContext workRunContext, WorkInstanceEntity workInstance) {
        ...
        try {
            // 上传脚本
            scpText(scpFileEngineNodeDto, workRunContext.getScript() + "\necho 'zhihuiyun_success'",
                clusterNode.getAgentHomePath() + "/zhihuiyun-agent/works/" + workInstance.getId() + ".sh");

            // 执行命令获取pid
            String executeBashWorkCommand = "nohup sh " + clusterNode.getAgentHomePath() + "/zhihuiyun-agent/works/"
                + workInstance.getId() + ".sh >> " + clusterNode.getAgentHomePath() + "/zhihuiyun-agent/works/"
        ...
        while (true) {
            ...
            if (InstanceStatus.RUNNING.equals(pidStatus)) {
                ...
                // 保存运行日志
                workInstance.setTaskManagerLog(logCommand.replace("zhihuiyun_success", ""));
                ...
                try {
                    String clearWorkRunFile = "rm -f " + clusterNode.getAgentHomePath() + "/zhihuiyun-agent/works/"
                        ...
                        + "/zhihuiyun-agent/works/" + workInstance.getId() + ".sh";
