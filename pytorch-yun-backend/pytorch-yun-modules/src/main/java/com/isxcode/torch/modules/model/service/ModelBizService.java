...
public class ModelBizService {
    ...
    //
    // // 检测数据源名称重复
    // Optional<DatasourceEntity> datasourceByName =
    // datasourceRepository.findByName(addDatasourceReq.getName());
    ...
    // // 检测数据源名称重复
    // Optional<DatasourceEntity> datasourceByName =
    // datasourceRepository.findByName(updateDatasourceReq.getName());
    // if (datasourceByName.isPresent() &&
    // !datasourceByName.get().getId().equals(updateDatasourceReq.getId())) {
    // throw new IsxAppException("数据名称重复");
    // }
    //
    // DatasourceEntity datasource = datasourceService.getDatasource(updateDatasourceReq.getId());
    //
    // datasource = datasourceMapper.dasUpdateDatasourceReqToDatasourceEntity(updateDatasourceReq,
    // datasource);
    //
    // // 密码对成加密
    // datasource.setPasswd(aesUtils.encrypt(datasource.getPasswd()));
    ...
    // datasourceService.getDatasource(testConnectReq.getDatasourceId());
    //
    // // 测试连接
    ...
    //
    // DatasourceEntity datasourceEntity =
    // datasourceMapper.checkConnectReqToDatasourceEntity(checkConnectReq);
