...
public class RunAgentInstallService {
    ...
    public void installAgent(ScpFileEngineNodeDto scpFileEngineNodeDto, ClusterNodeEntity engineNode,
        ...
        // 异步上传安装包
        clusterNodeService.scpAgentFile(scpFileEngineNodeDto, "classpath:agent/zhihuiyun-agent.tar.gz",
            sparkYunProperties.getTmpDir() + File.separator + "zhihuiyun-agent.tar.gz");
        log.debug("代理安装包上传中");

        // 同步监听进度
        clusterNodeService.checkScpPercent(scpFileEngineNodeDto, "classpath:agent/zhihuiyun-agent.tar.gz",
            sparkYunProperties.getTmpDir() + File.separator + "zhihuiyun-agent.tar.gz", engineNode);
        log.debug("下载安装包成功");
