package com.isxcode.torch.modules.work.run;

import com.alibaba.fastjson.JSON;
import com.isxcode.torch.api.agent.constants.AgentApi;
import com.isxcode.torch.api.agent.pojos.req.AcornPluginReq;
...
import com.isxcode.torch.api.work.pojos.res.RunWorkRes;
...
import com.isxcode.torch.modules.func.mapper.FuncMapper;
...
public class FlinkSqlExecutor extends WorkExecutor {
    ...
    protected void execute(WorkRunContext workRunContext, WorkInstanceEntity workInstance) {
        ...
        // 开始构造flinkSubmit
        SubmitJobReq submitJobReq = SubmitJobReq.builder().entryClass("com.isxcode.torch.plugin.sql.execute.Job")
            .agentHomePath(engineNode.getAgentHomePath()).appResource("flink-sql-execute-plugin.jar")
            .appName("zhihuiyun-job").workInstanceId(workInstance.getId())
