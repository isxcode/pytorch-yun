...
if [ "$hasRole" = "no" ]; then
  json_output="{ \
                \"status\": \"INSTALL_ERROR\", \
                \"log\": \"zhihuiyun没有创建pod的权限，需要执行命令，kubectl create clusterrolebinding spark-role --clusterrole=edit --serviceaccount=zhihuiyun-space:zhihuiyun --namespace=zhihuiyun-space \" \
              }"
  echo $json_output
  rm ${BASE_PATH}/agent-kubernetes.sh
  exit 0
